...
redis_server_stream = spark.readStream.format("kafka") \
...
redis_server_stream = redis_server_stream.select(col("value").cast("string"))
...
redis_server_stream.withColumn("data", from_json(col("value"), redis_server_schema)).select(col("data.*")).createOrReplaceTempView("RedisSortedSet")
...
DecodedCustomerDF = EncodedCustomerDF.withColumn("customer_data", unbase64(col("encodedCustomer")).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
DecodedCustomerDF.withColumn("customer_data_parsed", from_json(col("customer_data"), customer_schema)).select(col("customer_data_parsed.*")).createOrReplaceTempView("CustomerRecords")
...
emailAndBirthYearStreamingDF = emailAndBirthDayStreamingDF.select(col("email"), split(col("birthDay"), "-").getItem(0).alias("birthYear"))
...
joinedDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, on = (customerRiskStreamingDF.customer == emailAndBirthYearStreamingDF.email))
joinedDF.writeStream.format("console").outputMode("append").start()
...
joinedDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value").writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "stedi.data-to-plot")\
